...
class Php7Handler extends CBaseHandler{
	...
	public function __construct($app) {
		$this->_app = $app;
		$this->left_inner = 'php7_tasklist.tpl.php';
		$this->right_inner = 'php7_inner.tpl.php';
		$this->css[] = 'qs';
		...
		if (count($a) > 1) {
			$s = str_replace('/', '', $a[1]);
			if (file_exists(APP_ROOT . '/files/php7/' . $s . '.php')) {
				$this->book_tpl = $s;
	...
	*/
	static public function part($key, $href = false) {
		...
		return '<li><a class="" href="'. WEB_ROOT. '/php7/'. $key . $end_link . '">'. $lang[$display_text_key] . '</a></li>';
	}
	/**
	...
	*/
	static public function prevnext($keyprev, $keynext){
		...
		if ($keyprev) {
			$s .= '<div class="left">' . Php7Handler::aback($keyprev) . '</div>';
		}
		if ($keynext) {
			$s .= '<div class="right">' . Php7Handler::anext($keynext) . '</div>';
		}
