...
<p>То есть блок</p>
<p><b>if</b> ( r = <u title="Получить наименьший делитель">getMinimumDivider</u>(n) ) {...</p>
<p>работает так: вычисляется <u title="Получить наименьший делитель">getMinimumDivider</u>(n), вычисленное значение помещается в r, значение r обрабатывается условным оператором <b>if</b>.</p>
<h4>Вторая задача</h4>
...
<b>function</b> <u>example_6_2</u>() {
	...
	<b>var</b> n = <i>parseInt</i>( <u>readln</u>(<span class="strcolor">'Введите четное целое число N > 2'</span>) ),
		inputErrorMessage = <span class="strcolor">'Было введено недопустимое значение, выход'</span>,
		r, a, b;
	<b>if</b> (<i>isNaN</i>(n)) {
...
Также понятно, что недостаточно вычесть из четного n первое попавшееся простое число, чтобы осталось еще одно простое. 
...
Если функция getNextSimpleNumber получает аргумент меньше 41, она просто проходит по этому массиву, иначе хочется использовать формулу.
Но я могу это сделать когда n не больше чем значение F(39) = 1601, так как F(40) дает непростое число. 40 * 40 + 40 + 41 = 1681.
Также, мне необходимо найти x чтобы получить F(x + 1). Значит, мне нужна еще одна функция, решающая квадратное уравнение.
...
<p>После того, как обе вспомогательные функции реализованы, поиск простых чисел , сумма которых дает заданное n становится простым. Я отнимаю от n наименьшее простое число 2 и запускаю цикл, который выполняется до тех пор, пока оба слагаемых не станут простыми числами.
...
<p>Я специально использовал при определении (инициализации) переменных n и s конструкторы, чтобы убедится, что в данном случае их использование или не использование не играет никакой роли (то есть пример работает так же, если заменить n = <b>new</b> <b>Number</b>(5); на n = 5;  и s = <b>new</b> <b>String</b>(<span class="strcolor">'Hello'</span>); на s = <span class="strcolor">'Hello'</span>;).
...
<p>Привожу аргумент к целому числу. Если это вдруг оказалось не число, принимаю его равным нулю. Если число меньше двух, возвращаю единицу, так как 0! равен 1. Далее от единицы до n включительно умножаю значение i на результат и запоминаю это произведение в результате. Результат возвращаю, это факториал числа n.</p>
...
<p>Здесь я пользуюсь тем, что факториал любого числа n равен n умноженное на факториал n - 1. При этом факториал нуля равен 1, единицы соответственно тоже (1 * 0! = 1). Поэтому я внутри функции <u>recursiveFactorial</u> беру аргумент, и умножаю его на результат вызова функции recursiveFactorial которой передаю свой аргумент минус единица.</p>
...
просто переставить два числа в исходном множестве, а потом передать полученное сочетание в рекурсивную функцию, которая сделает то же самое, но будет работать не со всеми элементами множества, а с правыми n - 1 элементами.
...
<p>В начале определения функции <u>combinations</u> происходит проверка корректности ввода. Если данные корректны, они помещаются в массив array. Затем следует определение функции <u>writeVariants</u> 
...
<p>В функции <u>writeVariants</u> определен цикл, внутри которого происходит перестановка элементов. Чтобы сдвинуть все элементы массива влево, помещая вышедшей за пределы массива элемент в конец, нужно пройти по массиву из N элементов N*N раз. 
...
<p>Далее я добавил рекурсивный вызов функции <u>writeVariants</u>. Алгоритм сработал, но вывел много дублей. Избежать их удалось добавив условие перед печатью элементов массива: выводим последнюю полученую в цикле последовательность только в том случае, если отступ n равен 0. В этом случае вложенный вызов <u>writeVariants</u> не выводит ту последовательность, которая пришла к нему на вход (а этого не надо делать, потому что мы ее уже напечатали перед вложенным вызовом).</p>
...
Модифицирую алгоритм <u>writeVariants</u> так, чтобы функция не просто выводила результаты, но и проверяла их на правильность.
			...
			<b>if</b> (j == array.length - 1) {
				...
				<b>if</b> (i < lim - 1 || n == 0) {
					...
					<b>if</b> (ctrlUnique[a] == 1) {
						<b>throw</b> <b>new</b> <b>Error</b>(<span class="strcolor">"Дубль в выводе!"</span>);
					}
					ctrlUnique[a] = 1;
					ctrlLength.val++;
				}
...
<p>Я добавил два аргумента функции <u>writeVariants</u> - объект в котором буду хранить уже найденные сочетания и объект, в котором буду хранить записанное число сочетаний. Всякий раз после того, как выведено очередное сочетание, в объект ctrlUnique добавляется поле с именем, равным строке, представляющей собой это сочетание. Но перед этим проверяет, нет ли уже в объекте поля с таким именем.
