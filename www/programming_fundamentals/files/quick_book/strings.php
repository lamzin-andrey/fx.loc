...
<b>function</b> textCorrector() {
	...
	<b>var</b> endSimbols = <span class="strcolor">'.!?'</span>,     <span class="strcolor">//символы, которыми может заканчиваться предложение
		...
		char = text.<i title="Возвращает из строки смвол номер такой-то">charAt</i>(i);<span class="strcolor">//получили очередной символ.
			...
			char = <span class="strcolor">' '</span>;<span class="strcolor">//присваиваем символу пробел, так как слова и предложения по условию задачи должны разделяться пробелами
				...
				previousChar = text.<i title="Возвращает из строки смвол номер такой-то">charAt</i>(i - 1);
			}
...
<p>В первых строках программы (в первом цикле while) я показываю диалог для ввода строки текста. И продолжаю это делать до тех пор, пока пользователь не введет специальную строку endtext (я выбрал команду endtext, но в принципе это могло бы быть любым значением).</p>
<p>Каждую введенную пользователем строку я прийепляю к переменной text. Когда этот процесс завершен, я перехожу к анализу строки. Я немного упростил себе задачу, прняв, что слова могут состоять из любых символов кроме пробела, символа разрыва строки \n и символа табуляции \t. По-хорошему надо было бы дополнить этот набор всевозможными скобками и кавычками, но мне что-то сегожня лень ). Энтузиасты могут заняться этим самостоятельно.</p>
...
<p>Далее, я прохожу по введенному тексту от начала до конца (я использую свойство length объекта класса String чтобы определить длину текста в символах). Позиции символов нумеруются с нуля до (length - 1), поэтому условие окончания цикла записано i < text.length  а не i <= text.length.</p>
<p>Чтобы получить очередной символ строки я использую метод типа объекта String сharAt. Описание всех свойств и методов, которые есть у переменных (объектов) типа String вы можете прочесть на сайте <a href="http://javascript.ru/String" target="_blank">javasript.ru</a>.</p>
...
</span>		head = inputText.<i>substring</i>(0, middle), <span class="strcolor">//подстрока от начала до середины
</span>		tail = inputText.<i>substring</i>(middle); <span class="strcolor">//пдстрока от середины до конца
</span>	head = head.<i>replace</i>(<span class="strcolor">':'</span>, <span class="strcolor">';'</span>); <span class="strcolor">//заменили  двоеточие на точку с запятой
</span>	tail = tail.<i>replace</i>(<span class="strcolor">'.'</span>, <span class="strcolor">'!'</span>); <span class="strcolor">//заменили  точку на восклицательный знак
</span>	<u>writeln</u>(head + tail);
}
</pre>
<p>Здесь первым делом я определил какую-то строку, содержащую двоеточие и точку.</p>
<p>Затем, я использовал округление вниз числа, равного длине строки разделенной на два, чтобы определить середину строки. Далее, я использовал метод substring чтобы получить первую половину строки и вторую. Я поместил их в переменные, которые назвал "голова" и "хвост" (head и tail). Ну и в конце концов я вызвал для этих переменных метод String replace, который возвращает значение той переменной, для которой он вызван, заменив однако подстроку переданную в первом аргументе на подстроку переданную во втором аргументе.</p>
...
</span>	head = head.<i>replace</i>(<span class="recolor">/:/g</span>, <span class="strcolor">';'</span>); <span class="strcolor">//заменили все двоеточия на точки с запятыми
</span>	tail = tail.<i>replace</i>(<span class="recolor">/\./g</span>, <span class="strcolor">'!'</span>); <span class="strcolor">//заменили все точки на восклицательные знаки
</span>	<u>writeln</u>(head + tail);
}
</pre>
...
<p>Дело в том, что для чтения введенной пользователем переменной k мы используем parseInt, чтобы обеспечить контроль за вводимым значением. Благодаря этому в переменную k записывается объект типа <b>Number</b>. Но у таких объектов нет свойства length, которое можно использовать, чтобы определить количество знаков в числе. Поэтому я вызываю <b class="tblack">конструктор</b> <b>String</b> и передаю ему наше число k. В результате в переменную s запишется строка, а для неё мы уже можем вызвать метод String charAt.</p>
<div class="ainfo">Заодно здесь можно разъяснить что такое "литерная переменная" из текста задачи. Дело в том что эти задачки писались для выполнения на языках программирования Pascal или C++. В этих языках нельзя, как в JavaScript объявить переменную, а потом хранить в ней значение любого типа. Там если вы объявили переменную типа строка, знаит в ней могут быть только строки, если объявил переменную типа целое число, значит в ней могут быть только целые числа. А если вам надо хранить один символ строки, для этого можно использовать переменные специального типа, который и назван литерным.</div>
<p></p>
<div style="width:96%">
