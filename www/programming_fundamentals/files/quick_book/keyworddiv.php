...
for (var i = 0; i < arr.length; i++) {
	if (array[i].<i title="Возвращает позицию первого вхождения подстроки - аргумента в строку, для которой вызвана функция">indexOf</i>("Труба") == -1) {
...
<b title='//Буквально "переключатель". 
	switch (surname) {
		case "Гагарин":
			alert("Первый космонавт");
			break; //Если не ставить break и surname == "Гагарин" то alert про президента тоже вылезет
		case "Путин":
			alert("Президент");
			break; //Если не ставить break и surname == "Путин" то alert "Не знаю, кто это" тоже вылезет
		default:  //"Не знаю, кто это" вылезет, если surname не равно ни "Путин" ни "Гагарин"
			writeln("Не знаю, кто это.");
			break;
	}

'>switch</b>  <b title='//Буквально "бросать". Пример см. в подсказке для Error
