...
<p>Программка работает, однако, здесь у меня много повторяющегося кода. Преобразование позиции фигур в координаты массива chessBoard просится в отдельную функцию, с восемью очень похожими друг на друга участками кода проверки не вышел ли конь за пределы доски и не съел ли ферзя тоже надо что-то делать.</p>
...
<b>function</b> <u title="Пример удаления строки и столбца">removeRowAndColumnExample</u>() {
	<b>var</b> array = [], i, j, 
		...
		<b>for</b> (j = 0; j < ARRAY_SIZE; j++) {
			...
			m = m.<i>charAt</i>(3) ? m.<i>charAt</i>(3) : <span class="strcolor">'0'</span>; <span class="strcolor">//Почему именно третий? На самом деле от фонаря, мне ведь не важно что конкретно будет в массиве
</span>			array[i].push(m);
		}
	}
