...
<p>Ранее я походя упомянул, что переменные могут содержать не только числа.</p>
...
<p>Практически это значит, что если мы определили в программе переменную какого-то типа, мы можем использовать методы объекта этого типа применительно к значению этой переменной:</p>
<p>
	<pre>
		<b>function</b> <u title='пример использования метода'>useMethodExample</u>() {
			...
			<b title='В эту перемнную запишется массив созданный методом String.split'>var</b> arr = s.<i title='Разбивает строку на несколько по символу или строке, переданной в качестве аргумента. \n - символ окончания строки'>split</i>("\n");//Использовали метод String.split
...
<p>
	<pre>
		<b>function</b> <u title='Пример сравнения'>compareExample</u>() {
			<b>var</b> s = '0', n = 0;
			<i>alert</i>(s == n); //Сравнили строку в которой записан 0 с числом 0 - получили true, потому что операция == сравнивает только значения
