...
<p>От нас требуют найти сумму членов последовательности, каждый член которой, за исключением первого, вычисляется по формуле единица деленная на (а в степени 2*n - 2). n при этом равно номеру слагаемого, при нумерации их с единицы. Понятно, что чем больше n, тем больше делитель очередного слагаемого. Если вспомнить пределы последовательностей и функций, можно сказать, что делитель стремится к бесконечности, при n стремящемся к бесконечности.</p>
<p>Еще можно вспомнить, что единица, деленная на бесконечность равна нулю.</p>
<p>Решение этой задачи на языке JavaScript облегчается тем, что числовая переменная в JavaScript может принимать специальное значение Infinity - бесконечность.</p>
...
<p>В коде примера я получаю от пользователя значение a и контролирую, не равен ли он нулю или единице.</p>
...
<p>Данный пример очень похож на предыдущий, но с той поправкой, что элемент ряда никогда не станет равен 0.</p>
<p>Если не знаем, что такое n! смотрим в поисковой системе [подставьте ту, которой пользуетесь], что это такое и узнаем что это факториал. И даже, что факториал целого числа n равен произведению целых членов ряда [1 .. n] то есть все числа от одного до n включительно надо перемножить, тогда получим факториал.</p>
<p>Я вынес этот нехитрый алгоритм в функцию factorial и использую ее при решении основной задачи.</p>
<p>Если этот текст будут читать программисты, наверное скажут, что неплохо бы получить факториал через рекурсивные вызовы. Но на мой вхшгляд эта статья и так перегруженна, тем ьолее что то что я собираюсь сообщить в ней еще на мой взгляд не менее важно, чем рекурсивные алгоритмы.</p>
<h4>Производительность циклов. Что неплохо знать о циклах JavaScript программисту, стремящемуся писать код грамотно</h4>
(Излагаю по книге Стояна Стефанова &laquo;JavaScript. Шаблоны&raquo;)
...
<pre>
<b>for</b> (<b>var</b> i = array.length; i--) { <span class="strcolor">//Будет выполняться, пока i > 0, сравнение с нулем происходит быстрее чем с любым другим числом
</span>	<span class="strcolor">//тут что-то делаем
</span>}
</pre>
