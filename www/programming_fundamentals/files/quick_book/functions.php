...
<p>Итак, что это такое и для чего это нужно? Проще начать с того, для чего это нужно. Чаще всего функции в программировании используются для описания действий, которые в ходе работы программы должны выполнятся неоднократно. 
...
Она будет принимать два аргумента, описывающих щары, для которых надо проверить, столкнулись они или нет.
...
<b>function</b> <u title='main значит "главный"'>main</u>() {//главная функция
	...
	<b title='Вернет истину если шары столкнулись и ложь если это не так'>function</b> <u title='Пересекаются?'>isIntersect</u>(ball_1, ball_2) {
		...
		<b>var</b> a = ball_1, b = ball_2,
			d = <b>Math</b>.<i title='Квадратный корень'>sqrt</i>( sq(a.x - b.x) + sq(a.y - b.y) + sq(a.z - b.z) );
		<b>if</b> (d < 2*a.r) {//если расстояние между центрами шаров менбше чем диаметр шара
